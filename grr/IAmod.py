...
class IAmod(object):
    ...
    def simulate(self, V0, Vin, dt=1e-3, random_seed=42):
        ...
        for t in range(1, len(V_mat)):
            ...
            V_mat[t] = (
                V_mat[t - 1, :]
                + (dV * dt + V_noise[t - 1, :] * np.sqrt(dt)) / tau_eff
            )
    ...
    def ss_clamp(self, V):
        ...
        return (V - self.El) + self.ga * self.m_inf(V) * self.h_inf(V) * (
            V - self.Ea
        )
...
class Simulation(object):
    ...
    def PSTH(self, window_width):
        ...
        """
        kernel = np.ones(int(window_width / self.dt)) / (
            window_width * self.no_neurons
        )
    ...
    def simple_plot(self):
        ...
        raster_ax.plot(
            where_spks[0] * self.dt, where_spks[1], 'k|', markersize=0.5
        )
