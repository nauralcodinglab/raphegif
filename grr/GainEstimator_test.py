...
class GainEstimatorAttributesTests(unittest.TestCase):
    def setUp(self):
        ...
        self.gainEstimator.measuredResponse = np.zeros(
            (
                self.expectedNumReps,
                self.expectedNumSweeps,
                self.expectedNumTimeSteps,
            )
        )
...
class GainEstimatorFitTests(unittest.TestCase):
    def setUp(self):
        ...
        self.expectedGain = 5.0
        self.expectedIntercept = 3.0
        self.expectedGainUncertainty = 2.0
        ...
        self.baselineInterval = (0, 5.0)
        self.stimulusInterval = (6.0, 8)

        self.inputAmplitudes = np.linspace(1.0, 10.0, self.numSweeps)
    ...
    def _drawSampleResponses(self):
        ...
        """
        randomGains = np.random.normal(
            self.expectedGain, self.expectedGainUncertainty, self.numReps
        )
        randomIntercepts = np.random.normal(
            self.expectedIntercept,
            self.expectedInterceptUncertainty,
            self.numReps,
        )

        sampleResponses = np.zeros(
            (self.numReps, self.numSweeps, self.numTimeSteps)
        )
        for i in range(self.numReps):
            values = np.polyval(
                (randomGains[i], randomIntercepts[i]), self.inputAmplitudes
            )
            sampleResponses[i, :, self.stimulusStartInd :] = values[
                :, np.newaxis
            ]
    ...
    def testExpectedGain(self):
        ...
        self.gainEstimator.fit(
            sampleResponses,
            self.inputAmplitudes,
            self.baselineInterval,
    ...
    def testExpectedGainUncertainty(self):
        ...
        npt.assert_allclose(
            ...
            np.broadcast_to(
                self.expectedGainUncertainty,
                len(self.gainEstimator.gainUncertainty),
            ),
            atol=0,
            rtol=0.2,
    ...
    def testExpectedIntercept(self):
        ...
        npt.assert_allclose(
            ...
            np.broadcast_to(
                self.expectedIntercept, len(self.gainEstimator.intercept)
            ),
            atol=0,
            rtol=0.2,
    ...
    def testExpectedInterceptUncertainty(self):
        ...
        npt.assert_allclose(
            ...
            np.broadcast_to(
                self.expectedInterceptUncertainty,
                len(self.gainEstimator.interceptUncertainty),
            ),
            atol=0,
            rtol=0.2,
