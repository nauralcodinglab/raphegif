...
class Filter :
    ...
    def clearInterpolatedFilter(self):

        """
        Clear cached interpolated filter as well as its support.

        The interpolated filter is stored in a class attribute along with a
        corresponding support vector. If the filter is large, this can take up
        significant disk space when the model is saved. Clearing the cached
        filter avoids this problem and has no adverse effects because the filter
        can be reinterpolated easily.
        """

        for attr_ in ('filtersupport', 'filter'):
