...
for i, expt in enumerate(experiments):
    ...
    KGIFs.append(tmp_GIF)
    tmp_GIF.printParameters()

#%%
from copy import deepcopy

tmp = deepcopy(KGIFs[0])

#%%

tmp = AugmentedGIF(0.1)

experiment_no = 0

tmp.fitVoltageReset(experiments[experiment_no], 6.5, True)
tmp.fitSubthresholdDynamics(experiments[experiment_no], 1.5)
t, V, eta_sum = tmp.simulateDeterministic_forceSpikes(
    experiments[experiment_no].trainingset_traces[0].I,
    experiments[experiment_no].trainingset_traces[0].V[0],
    experiments[experiment_no].trainingset_traces[0].getSpikeTimes()
)

plt.figure(figsize = (10, 5))
ax = plt.subplot(311)
plt.title('Input current')
plt.plot(t, experiments[experiment_no].trainingset_traces[0].I)
