...
"""
Very little of KappaComparator is actually used for this analysis, but the class
...
Flo, Vlo, Ilo = gk2_comparator.get_FVI(1)
freq_cutoff = 1e2
freq_sub = np.min(np.where(Flo[:, 0] >= freq_cutoff)[0])
Flo = Flo[:freq_sub, :]
Vlo = Vlo[:freq_sub, :]
Ilo = Ilo[:freq_sub, :]

Fhi, Vhi, Ihi = gk2_comparator.get_FVI(2)
freq_sub = np.min(np.where(Fhi[:, 0] >= freq_cutoff)[0])
Fhi = Fhi[:freq_sub, :]
Vhi = Vhi[:freq_sub, :]
Ihi = Ihi[:freq_sub, :]
...
spec = mpl.gridspec.GridSpec(1, 3, top = 0.9, right = 0.95, left = 0.05, bottom = 0.15)

plt.subplot(spec[0, 0])
outliers = [3, 12]
gk2_color = (0.9, 0.2, 0.2)
plt.title('\\textbf{{A}} Distribution of $\\bar{{g}}_{{Kslow}}$', loc = 'left')
plt.hist(
...
ax0 = plt.subplot(spec[0, 1], projection = '3d')
ax0.set_title('\\textbf{{B1}} Temporal filter with low $K_{{slow}}$ (1pS)', loc = 'left')
ax0.plot_surface(np.log10(Flo), Vlo, Ilo, rstride = 1, cstride = 1, cmap = cm.coolwarm, linewidth = 0, antialiased = False)
ax0.set_ylim3d(ax0.get_ylim3d()[1], ax0.get_ylim3d()[0])
ax0.set_xticks([-1, 0, 1, 2])
ax0.set_xticklabels(['$10^{{{}}}$'.format(tick) for tick in ax0.get_xticks()])
ax0.set_yticks([-45, -55, -65, -75])
ax0.set_xlabel('$f$ (Hz)')
ax0.set_ylabel('Vm (mV)')
ax0.set_zlabel('Impedance (GOhm)')
ax0.xaxis.labelpad = 15
ax0.yaxis.labelpad = 15
ax0.zaxis.labelpad = 15

ax1 = plt.subplot(spec[0, 2], projection = '3d')
ax1.set_title('\\textbf{{B2}} Temporal filter with high $K_{{slow}}$ (15pS)', loc = 'left')
ax1.plot_surface(np.log10(Fhi), Vhi, Ihi, rstride = 1, cstride = 1, cmap = cm.coolwarm, linewidth = 0, antialiased = False)
ax1.set_ylim3d(ax1.get_ylim3d()[1], ax1.get_ylim3d()[0])
ax1.set_xticks([-1, 0, 1, 2])
ax1.set_xticklabels(['$10^{{{}}}$'.format(tick) for tick in ax1.get_xticks()])
ax1.set_yticks([-45, -55, -65, -75])
ax1.set_xlabel('$f$ (Hz)')
ax1.set_ylabel('Vm (mV)')
ax1.set_zlabel('Impedance (GOhm)')
ax1.xaxis.labelpad = 15
ax1.yaxis.labelpad = 15
ax1.zaxis.labelpad = 15

#plt.savefig(IMG_PATH + 'newfig6_membraneFilter.png', dpi = 300)
