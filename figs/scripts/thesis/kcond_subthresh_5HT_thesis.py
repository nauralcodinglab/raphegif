...
spec_outer = gridspec.GridSpec(
    3, 1, left = 0.05, right = 0.95, top = 0.95, bottom = 0.1, hspace = 0.75
)
subfig_hspace = 1.3
subfig_wspace = 0.4
...
spec_C = gridspec.GridSpecFromSubplotSpec(
    2, 3, spec_outer[2, :], width_ratios = [1.1, 1, 0.5],
    hspace = subfig_hspace, wspace = subfig_wspace
)
...
plt.subplot(spec_C[:, 0])
plt.title('\\textbf{{C1}} Both conductances', loc = 'left')
testset_traces_plot(
gk2_mod_coeffs, full_mod_coeffs, 13,
null_mod_label = 'Linear + $K_{{\mathrm{{slow}}}}$',
alt_mod_label = 'Linear + both',
...
plt.subplot(spec_C[:, 1])
plt.title('\\textbf{{C2}} Binned test set error', loc = 'left')
binned_e_comparison_plot(
gk2_mod_coeffs, full_mod_coeffs,
'Linear + $K_{{\mathrm{{slow}}}}$', 'Linear + both',
null_color = Kslow_color,
alt_color = np.mean([Kfast_color, Kslow_color], axis = 0)
)
...
plt.subplot(spec_C[0, 2])
plt.title('\\textbf{{C3}}  -60mV', loc = 'left')
...
plt.subplot(spec_C[1, 2])
plt.title('\\textbf{{C4}}  -45mV', loc = 'left')
