    ...
    for i, pred_pair in enumerate(predictors):
        ...
        for V0 in V0_vec:
            IA_predictions.append(
                pred_IA.predict_spk(pred_IA.gaprime_est, pred_IA.thresh_est, V0, pred_IA.Vinput_est, pred_IA.tauh_est, max_time = 15)
            )

        plt.plot(pred_IA.V0, pred_IA.spks, 'k.')
        plt.plot(V0_vec, np.array(IA_predictions) * pred_IA.tau_est, 'b--', label = 'IA')
