...
plt.plot(
    1e-3 * np.where(low_IA_sim.spks[:, :neurons_to_show])[0],
    np.where(low_IA_sim.spks[:, :neurons_to_show])[1],
    'k|', markersize = 2
)
...
plt.plot(
    1e-3 * np.where(hi_IA_sim.spks[:, :neurons_to_show])[0],
    np.where(hi_IA_sim.spks[:, :neurons_to_show])[1],
    'k|', markersize = 2
)
pltools.add_scalebar(
