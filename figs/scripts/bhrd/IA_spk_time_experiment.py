...
class OhmicSpkPredictor(object):
    ...
    def scrape_data(self, V0_range = (2495, 2595), Vinf_range = (2750, 2800), baseline_range = (0, 100),
        quiescent_until = 2600, dVdt_thresh = 10, exclude_above = -55):
...
baseline_recs = inventory.loc[np.logical_and(inventory['Cell_ID'] == 'DRN332', inventory['4AP'] == 0), 'Recording']
_4AP_recs = inventory.loc[np.logical_and(inventory['Cell_ID'] == 'DRN332', inventory['4AP'] == 1), 'Recording']
...
pred_baseline.fit_spks(verbose = True, force_tau = np.mean(pred_baseline.taus), Vinf_guesses = np.linspace(-50, 50, 100))
pred_4AP.fit_spks(verbose = True, force_tau = np.mean(pred_4AP.taus), Vinf_guesses = np.linspace(-50, 50, 100))
...
plt.figure()
plt.plot(pred_4AP.V0, pred_4AP.spks, 'k.')
V0_vec = np.linspace(-100, -55, 200)
plt.plot(V0_vec, pred_4AP.predict_spks(V0 = V0_vec, Vinf = pred_4AP.Vinf_est), 'k--')
plt.show()
